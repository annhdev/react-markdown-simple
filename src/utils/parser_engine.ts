...
export const parseMarkdown = (text: string, extraPlugins: MarkdownPlugin[] = []) => {
    ...
    lines.forEach((line) => {
        ...
        while (blockquoteLevel < currentQuoteLevel) {
            output.push(`<blockquote class="blockquote border-l-2 border-gray-300 pl-4 py-1 my-2 italic text-gray-600 dark:text-gray-300 bg-gray-50 dark:bg-gray-500 ml-${blockquoteLevel * 2}" style="margin-left: calc(${blockquoteLevel} * 0.25rem)">`)
            blockquoteLevel++
        }
        while (blockquoteLevel > currentQuoteLevel) {
